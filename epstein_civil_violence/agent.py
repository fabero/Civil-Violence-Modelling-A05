...
class Citizen(Agent):
    ...
    def update_neighbors(self):
        ...
        self.neighborhood = self.model.grid.get_neighborhood(self.pos,
                                                        moore=False, radius=self.vision)
    ...
    def update_estimated_arrest_probability(self):
        ...
        self.arrest_probability = 1 - math.exp(
            -1 * self.model.arrest_prob_constant * (
               int(cops_in_vision / actives_in_vision)))
        #if cops_in_vision > actives_in_vision:
        #    self.arrest_probability = 1
        #else:
...
class Cop(Agent):
    ...
    def step(self):
        ...
        for agent in self.neighbors:
            ...
            if agent.breed == 'citizen' and \
                    agent.jail_sentence >0:
                jailed_neighbors.append(agent.pos)
        if active_neighbors:
